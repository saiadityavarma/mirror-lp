...
export default function FrameworkSelectPage({ onSelect, onViewGraph }: Props) {
  ...
  useEffect(() => {
    ...
    getQuestions()
      .then((qs) => setSavedCount(qs.length))
      .catch(() => {});
  }, []);

  return (
    <div
      className="min-h-screen bg-slate-950 flex flex-col items-center justify-center px-6 py-12 overflow-y-auto"
    ...
    >
      ...
      {/* Return to graph banner */}
      {savedCount > 0 && (
        <button
          onClick={onViewGraph}
          className="w-full max-w-3xl mb-6 flex items-center justify-between px-5 py-4 rounded-xl border border-indigo-700 bg-indigo-950/50 hover:bg-indigo-900/50 transition-colors"
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <div className="text-indigo-400 text-xs mt-0.5">
                {savedCount} reflection{savedCount !== 1 ? "s" : ""} saved
              </div>
            </div>
          </div>
          <span className="text-indigo-400 text-sm">View graph →</span>
        </button>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 w-full max-w-3xl">
        {frameworks.map((fw) => {
          // Count saved answers for this framework via localStorage
          const answeredRaw =
            typeof window !== "undefined"
              ? localStorage.getItem(`mirror_answered_${fw.id}`)
              : null;
          const answeredCount = answeredRaw
            ? JSON.parse(answeredRaw).length
            : 0;
          const remaining = fw.principle_count - answeredCount;
          ...
          return (
            ...
            >
              ...
              >
                {answeredCount > 0
                  ? `${answeredCount} answered · ${remaining} remaining →`
